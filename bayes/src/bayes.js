export const simpleTokenizer = (string) => {
...
class BayesClassifier {
     ...
     * If null is passed as a the label, return the total number of training documents seen.
     * @param {string} label 
     */
     ...
     * If no label is given, returns te total number of times the token was seen 
    ...
    /**
     * Give a token stream (ie a tokenized document), calculate the probability that
    ...
    calculateLabelProbability(label, tokens) {
        ...
        const tokenScore = tokens.map(token => this.calculateTokenScore(token, label))
            .filter(score => Math.abs(probLabel - score) > epsilon);

        // to avoid floating point underflow when working with really small number, 
        ...
        const logSum = tokenScore.reduce((sum, score) => sum + (Math.log(1 - score) - Math.log(score)), 0);
